...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom Colliders via the ImDrawList API
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList();                                            // this draw list will be the first rendering one. Useful to quickly draw Colliders/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList();                                            // this draw list will be the last rendered one. Useful to quickly draw Colliders/text over dear imgui contents.
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedFill;            // Enable anti-aliased edges around filled Colliders (rounded rectangles, circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedFill         = 1 << 2,  // Enable anti-aliased edge around filled Colliders (rounded rectangles, circles).
...
//  - One or more fonts.
//  - Custom graphics data needed to render the Colliders needed by Dear ImGui.
//  - Mouse cursor Colliders for software cursor rendering (unless setting 'Flags |= ImFontAtlasFlags_NoMouseCursors' in the font atlas).
